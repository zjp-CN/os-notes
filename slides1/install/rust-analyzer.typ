...
#let install = [
  ...
  - 在其他编辑器上，你需要仔细阅读上面的手册链接，比如通过
    `rustup component add rust-analyzer` 命令安装它，并安装相关的编辑器插件。 #footnote[
    我是 NeoVim 的重度使用者，几乎每天都通过 NeoVim 编码。如果你想使用它的话，可以参考我的#link("https://github.com/zjp-CN/nvim-config")[配置文件]。
  ]
